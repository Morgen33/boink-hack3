-- Create enum-like constraint for conversation_context (if not exists)
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'conversation_context_check') THEN
...
-- Create enum-like constraint for created_from_intent (if not exists)  
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'created_from_intent_check') THEN
        ALTER TABLE public.conversations 
...
created_from_intent = CASE
  ...
  ELSE 'both'
END
WHERE conversation_context = 'mixed' AND created_from_intent = 'both';
